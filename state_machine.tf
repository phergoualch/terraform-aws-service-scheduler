...
locals {
  scheduler_is_true_conditon = {
    StringEquals = {
      "aws:ResourceTag/${var.tags_prefix}:${var.tags_mapping["enabled"]}" = "true"
    }
  }

  scheduler_is_not_false_conditon = {
    StringNotEquals = {
      "aws:ResourceTag/${var.tags_prefix}:${var.tags_mapping["enabled"]}" = "false"
    }
  }

  scheduler_conditon = var.schedule_without_tags ? local.scheduler_is_not_false_conditon : local.scheduler_is_true_conditon
}
        ...
        Statement = [
          {
            ...
            Effect    = "Allow",
            Resource  = "*",
            Condition = local.scheduler_conditon
          }
        ]
      })
    }
